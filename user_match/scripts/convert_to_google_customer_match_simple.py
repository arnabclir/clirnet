...
Simplified version using only standard library (no pandas required)
...
Usage:
    python convert_to_google_customer_match_simple.py <input_file> <output_file>

Example:
    python convert_to_google_customer_match_simple.py sample_doctor_data.csv hashed_doctor_data.csv
"""
...
class DoctorDataConverter:
    ...
    def print_summary(self):
        ...
        match_rate = (self.stats['valid_rows']/max(1, self.stats['total_rows'])*100) if self.stats['total_rows'] > 0 else 0
        self.log(f"Match rate estimate:  {match_rate:.1f}%")
        self.log("="*60)
        self.log("\n✅ CSV ready for Google Ads Customer Match upload!")
        self.log("   Minimum rows for matching: 1,000")
        if self.stats['valid_rows'] >= 1000:
            self.log("   Your file has: ✅ Good!")
        else:
            self.log(f"   Your file has: ⚠ {1000 - self.stats['valid_rows']} more rows needed")
        self.log("\n📋 Next steps:")
...
def main():
    """Main entry point."""
    if len(sys.argv) < 3:
        print("Usage: python convert_to_google_customer_match_simple.py <input_csv> <output_csv>")
        print("\nExample:")
        print("  python convert_to_google_customer_match_simple.py sample_doctor_data.csv hashed_doctor_data.csv")
        sys.exit(1)
